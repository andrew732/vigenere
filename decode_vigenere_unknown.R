...
codeword <- "lemonthereasonipickedlemonwasbecauseitwasanexample"
...
bestScore <- score(decrypt(codemess,"a"))
bestWord <- "a"
for(j in 1:(nchar(message))){
  curFunc <- strrep("a",j)
